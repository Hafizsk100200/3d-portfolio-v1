...
(async () => {
  ...
  try {
    ...
    await page.goto('http://localhost:4174/', { waitUntil: 'load', timeout: 15000 });
    ...
    const outputPath = path.join(artifactDir, 'v2_local_preview.png');
    await page.screenshot({ path: outputPath });
    console.log(`Saved local preview screenshot to ${outputPath}`);
  } catch (err) {
