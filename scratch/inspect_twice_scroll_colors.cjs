...
(async () => {
  ...
  const files = ['scroll_1_down.png', 'scroll_2_top.png', 'scroll_3_down.png'];
  ...
  const analyze = async (filename) => {
    ...
    const colors = await page.evaluate((base64) => {
      return new Promise((resolve) => {
        ...
        img.onload = () => {
          ...
          const leftData = ctx.getImageData(img.width * 0.1, img.height * 0.1, 1, 1).data;
          
          resolve({
            center: `rgb(${centerData[0]}, ${centerData[1]}, ${centerData[2]})`,
            topLeft: `rgb(${leftData[0]}, ${leftData[1]}, ${leftData[2]})`
          });
        };
    ...
    console.log(`[${filename}] Center: ${colors.center}, Top-Left: ${colors.topLeft}`);
  };
